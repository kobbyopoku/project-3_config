...
def job_config = [
        job: [
                name: "project-3_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'project-3_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kobbyopoku/project-3_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.project-3.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kobbyopoku", "project-3", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("project-3_Pipeline_develop");

job.with {

    authenticationToken('5SRE6e0MEKD5POObaXOC9yT8ATkv5L0a')
